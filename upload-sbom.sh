...
SBOM=$(echo "$SBOM" | sed 's:^/*::')

for var in "REPO" "BRANCH" "SBOM"; do
    if [ -z "${!var:-}" ]; then
...
case "${ON_FINDINGS:-fail}" in
    fail|alert|pass) ;;
    *)
    echo "Error: Invalid on-findings value: '${ON_FINDINGS}'. Must be one of: fail, alert, pass"
    exit 1
    ;;
esac
